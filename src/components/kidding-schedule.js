...
const KiddingSchedule = ({mate, name, img, notes, prices, kiddingDate, slug, pedigree, mateImg}) => (
	<tr>
	  ...
	  <td>
		{Boolean(notes?.length) && notes.map(note => <p className={styles.note} key={note}>{note}</p>)}
	  </td>
	  <td>
		{Boolean(prices?.length) && prices.map(price => <p className={styles.note} key={price}>{price}</p>)}
	  </td>
	</tr>
)
